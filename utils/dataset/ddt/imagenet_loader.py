...
from utils.ddt.ddt_augment import *
...
def load_val_bbox(label_dict,all_imgs,gt_location):
    ...
    files_xml = os.listdir(gt_location)
    final_bbox_dict = {}
    for xml in files_xml:
        dom = xml.dom.minidom.parse(os.path(gt_location,xml))
        root = dom.documentElement
        bbox = root.getElementsByTagName('bndbox')
        final_bbox_dict[xml[:-4]+'.jpg']=bbox
